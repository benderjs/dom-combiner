...
function combine(dest, src) {
    ...
    function find(collection, name) {
        return _.where(collection, { name: name })[0];
    }
    ...
    function appendTag(elem) {
        ...
        if (!elem.parent) {
            ...
            if (parent) parent = find(parent.children, 'body');
            if (parent) utils.appendChild(parent, elem);
    ...
    src.forEach(function (elem) {
        ...
        else if (name === 'html') merge(elem);
        else if (name === 'head' || name === 'body') mergeHeadBody(elem);
